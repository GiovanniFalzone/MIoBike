...
static void receiver(struct simple_udp_connection *c, const uip_ipaddr_t *sender_addr, uint16_t sender_port, const uip_ipaddr_t *receiver_addr, uint16_t receiver_port, const uint8_t *data, uint16_t datalen) {
	const char cmd_check_msg[8] = "get_GPS\0";
	char cmd_recv[8];
	strcpy(cmd_recv, data);
	cmd_recv[7] = '\0';
	#ifdef DEBUG_RECEIVE
		...
		printf(data); printf("\n");
		printf(cmd_recv); printf("\n");
		printf(cmd_check_msg); printf("\n");
		printf("%d\n", strcmp(cmd_recv, cmd_check_msg));
	#endif
	if(strcmp(cmd_recv, cmd_check_msg) == 0){
		struct Sensor_DATA Sensor_read = read_Sensor_data();
		send_unicast_sensor_data(Sensor_read, sender_addr);
	} else {
		char msg[16] = "unknown_cmd\0"; 
		simple_udp_sendto(&unicast_connection, (void*)msg, sizeof(msg), sender_addr);
	}
}
